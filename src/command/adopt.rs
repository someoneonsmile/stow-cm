...
pub async fn adopt(
    ...
    for source in &sources {
        ...
        if !config_path.exists() {
            generate_config(&config_path, pack_name, source).await?;
        ...
        let merge_result = {
            ...
            tokio::task::spawn_blocking(move || {
                MergeTree::new(&pack_dir, &target, Some(merge_option)).merge_add()
            })
            .await??
        };
        ...
        if let Some(ref to_create) = merge_result.to_create_symlinks {
            adopt_move_files(pack_name, to_create).await?;
        }

        // 复用 install 创建链接 + 写 track file + 执行 init 脚本
        let config = Arc::new(config);
        install::install(config, &pack_dir).await?;
    }
...
async fn generate_config(config_path: &Path, pack_name: &str, source: &Path) -> Result<()> {
    let content = format!(
...
async fn adopt_move_files(pack_name: &str, to_move: &[Symlink]) -> Result<()> {
    let pack_name = pack_name.to_owned();
    let to_move = to_move.to_vec();
